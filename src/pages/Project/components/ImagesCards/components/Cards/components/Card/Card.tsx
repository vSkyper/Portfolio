...
import { CardLoader, VideoIndicators } from './components';
...
export default function Card({ image, onOpen }: CardProps) {
  const isVideo = typeof image !== 'string';
  const thumbnailSrc = typeof image === 'string' ? image : image.thumbnail;

  const [imagesReady, setImagesReady] = useState(() => !thumbnailSrc);
  ...
  return (
    <>
      <m.div
        ...
        className="min-w-[70%] sm:min-w-[30%] h-0 pb-[45%] sm:pb-[18%] cursor-pointer group select-none relative rounded-2xl outline-none ring-1 ring-white/10 hover:ring-2 hover:ring-primary/50 focus-visible:ring-2 focus-visible:ring-primary/50 transition duration-300 ease-out hover:shadow-[0_20px_40px_-12px_rgba(122,181,220,0.3)] shadow-[0_10px_30px_-10px_rgba(0,0,0,0.8)] will-change-transform overflow-hidden bg-white/5"
        role="button"
        tabIndex={0}
        onKeyDown={(e) => {
      ...
      >
        ...
        {isVideo ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-5">
            <div className="w-12 h-12 sm:w-14 sm:h-14 bg-black/60 rounded-full flex items-center justify-center ring-1 ring-white/20 shadow-lg scale-75 opacity-0 translate-y-4 group-hover:scale-100 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 ease-out">
